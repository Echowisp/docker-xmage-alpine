#!/bin/sh
# update version
XMAGE_CONFIG=/xmage/xmage/mage-server/config/config.xml
...
java -Xms2G -Xmx2G -Djava.security.policy=./config/security.policy -Djava.util.logging.config.file=./config/logging.config -Dlog4j.configuration=file:./config/log4j.properties -jar ./lib/mage-server-*.jar
